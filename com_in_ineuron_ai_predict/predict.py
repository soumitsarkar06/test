...
class LogoClassification:
    ...
    def getPrediction(self, img):
        # img = cv2.imread(img) # Commented by Soumit
        # img = Image.open(img) # Added by Soumit
        dim = (224, 224)
        # img = img.resize(dim) # Added by Soumit
        img = image.load_img(img, target_size=dim) # added by Soumit
        # img = cv2.resize(img, dim, interpolation=cv2.INTER_AREA) # Commented by Soumit
